...
function Chatbot() {
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      // ... (Gemini API call remains unchanged)
    } catch (error) {
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      ...
      setMessages([
        {
          sender: "bot",
          text: `Thank you, ${contactForm.name}! We have received your inquiry: "${contactForm.message}". How can I further assist you today?`,
        },
      ]);
    } catch (error) {
  ...
  const renderHome = () => (
    <div
      ...
      style={{
        height: "calc(100% - 64px)", // Accounts for bottom navigation
        scrollbarWidth: "thin",
    ...
    >
      ...
      <div className="mb-6 sticky -top-4 bg-[#060922] py-2 z-10">
    ...
    <div className="flex-1 flex flex-col h-full relative">
      {!userDetails ? (
        <form onSubmit={handleContactSubmit} className="p-4 space-y-4">
          ...
          <button
            type="submit"
            className="bg-[#09A8EB] cursor-pointer text-white p-2 rounded-lg hover:bg-[#09A8EB]/80 transition w-full"
          >
            Submit Inquiry
          </button>
        </form>
